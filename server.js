...
app.use((req, res, next) => {
    const allowedOrigins = [
      'https://prismatic-donut-dc7697.netlify.app', // ТВОЙ FRONTEND
      'http://localhost:3000',
...
app.post('/api/create-test-data', (req, res) => {
    try {
        ...
        if (!users.find(u => u.username === 'test')) {
            ...
            users.push(testUser);
            writeData(USERS_FILE, users);
            console.log('✅ Создан тестовый пользователь: test/test');
        }

        // Создаем тестовые заявки если нет
        if (orders.length === 0) {
            const testOrders = [
                {
                    id: 'TEST001',
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🌐 Для продакшена: https://your-app.onrender.com`);
});
